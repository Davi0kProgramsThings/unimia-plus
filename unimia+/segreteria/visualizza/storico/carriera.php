<?php
    ...
    $matriculation = $_GET['studente'];

    if (!isset($matriculation)) {
        redirect('/segreteria/visualizza/storico/studenti.php');
    }

    if (!isset($_POST['tab']) || $_POST['tab'] == 0) {
        [ $rows, $_ ] = execute_query(
            'SELECT * FROM get_full_historic_career($1) ORDER BY date DESC', 
                [ $matriculation ]);
    }
    else if ($_POST['tab'] == 1) {
        [ $rows, $_ ] = execute_query(
            "SELECT * FROM get_full_historic_career($1) WHERE mark_status = 'A' ORDER BY date DESC", 
                [ $matriculation ]);
    }
?>
...
<head>
    ...
    <title>Unimia+ | Segreteria | Visualizza | Storico | Carriera</title>
</head>
            ...
            <div class="columns mb-0">
                <div class="column is-four-fifths">
                    <span class="icon-text mb-4">
                        ...
                        <h1 class="title mt-2">Visualizza storico carriera</h1>
                    </span>
                </div>
